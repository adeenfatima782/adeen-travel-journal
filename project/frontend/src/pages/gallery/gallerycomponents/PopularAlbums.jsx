    ...
    <div className="d-flex justify-content-between align-items-end mb-4">
      <div>
        <p className="font-mono text-gold-dark mb-2" style={{ fontSize: "0.75rem", letterSpacing: "0.2em", textTransform: "uppercase" }}>
          <i className="bi bi-fire me-1"></i>Most Viewed
        </p>
        <h2 className="font-display fw-semibold h3 mb-0">Popular Albums</h2>
      </div>
    </div>
    <div className="d-flex gap-3 overflow-auto pb-2" style={{ scrollSnapType: "x mandatory" }}>
      {albums.map((album) => (
        ...
        >
          ...
          <div className="p-3">
            <p className="font-mono text-gold-dark mb-1" style={{ fontSize: "0.68rem", textTransform: "uppercase" }}>{album.region}</p>
            <h3 className="h6 font-display fw-semibold mb-1">{album.name}</h3>
            <p className="font-mono text-stone mb-0" style={{ fontSize: "0.68rem" }}>
              <i className="bi bi-eye me-1"></i>{album.views.toLocaleString()} views
            </p>
          </div>
        </Link>
      ))}
