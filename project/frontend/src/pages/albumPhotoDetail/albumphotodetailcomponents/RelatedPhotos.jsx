...
const RelatedPhotos = ({ album, currentId }) => {
  const others = album.photos.filter((p) => p.id !== currentId).slice(0, 6);
    ...
    <section className="mt-5 pt-5 border-top" style={{ borderColor: "rgba(28,27,26,0.1)" }}>
      <div className="d-flex justify-content-between align-items-end mb-4">
        <p className="font-mono text-gold-dark mb-0" style={{ fontSize: "0.72rem", letterSpacing: "0.2em", textTransform: "uppercase" }}>
          More From {album.name}
        </p>
        <Link to={`/gallery/album/${album.slug}`} className="link-underline font-mono text-stone small">Back to album →</Link>
      </div>
      <div className="row g-4">
        {others.map((p) => (
          <div className="col-6 col-md-4" key={p.id}>
            <Link to={`/gallery/album/${album.slug}/${p.id}`} className="text-decoration-none text-reset d-block">
